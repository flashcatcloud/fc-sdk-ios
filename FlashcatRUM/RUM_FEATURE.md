...
### Feature Entry Point
- **`FlashcatRUM/Sources/RUM.swift`** - Main entry point. Call `RUM.enable(with:)` to activate the feature.

### Configuration
- **`FlashcatRUM/Sources/RUMConfiguration.swift`** - All configuration options available to customers.
...
### Public API
- **`FlashcatRUM/Sources/RUMMonitor.swift`** - Access point for manual RUM tracking via `RUMMonitor.shared()`
- **`FlashcatRUM/Sources/RUMMonitorProtocol.swift`** - Full API for manual RUM instrumentation
...
### Implementation
- **`FlashcatRUM/Sources/Feature/RUMFeature.swift`** - Internal feature implementation. Shows how configuration translates to behavior.
