...
public protocol FlashcatCoreProtocol: AnyObject, MessageSending, AdditionalContextSharing, Storage {
    // Remove `FlashcatCoreProtocol` conformance to `MessageSending` and `BaggageSharing` once
    ...
    func scope<T>(for featureType: T.Type) -> FeatureScope where T: FlashcatFeature
}

extension FlashcatCoreProtocol {
    /// Returns a `DatadogFeature` conforming type from the
    /// Feature registry.
    ///
    /// - Parameter type: The Feature instance type.
    /// - Returns: The Feature if any.
    public func get<T>(feature type: T.Type = T.self) -> T? where T: FlashcatFeature {
...
public extension FeatureScope {
    ...
    ///   - block: The block to execute; it is called on the context queue.
    func eventWriteContext(_ block: @escaping (FlashcatContext, Writer) -> Void) {
    ...
    ///
    /// - Parameter block: The block to execute; it is called on the context queue.
    func dataStoreContext(_ block: @escaping (FlashcatContext, DataStore) -> Void) {
        context { context in
...
public class NOPDatadogCore: FlashcatCoreProtocol {
    public init() { }
    /// no-op
